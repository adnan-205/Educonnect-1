...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Login with Google' button to initiate Google OAuth flow.
        frame = context.pages[-1]
        # Click on 'Login with Google' button to start Google OAuth flow
        elem = frame.locator('xpath=html/body/main/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the Google account email to proceed with OAuth authentication.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=User role: SuperAdmin').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Google OAuth login did not authenticate user correctly or assign the expected role. Expected user to be redirected to the correct role-based dashboard with a valid JWT token.")
