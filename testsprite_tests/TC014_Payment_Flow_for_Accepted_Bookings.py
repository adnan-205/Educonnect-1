...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Payment Confirmation: Transaction Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Payment transaction was not successful, booking not marked as paid, or teacher's earnings not updated as per the test plan.")
