...
export default function EditGigPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (gig.thumbnailUrl) {
          setThumbPreview(gig.thumbnailUrl)
        }
      } catch (e: any) {
  ...
  const onUploadThumb = async () => {
    ...
    try {
      ...
      const url = res?.data?.url || res?.gig?.thumbnailUrl || res?.url
      if (url) {
        ...
        setError("Upload succeeded but no thumbnail URL was returned")
      }
    } catch (e: any) {
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {(thumbPreview || form.thumbnailUrl) && (
                  <Button type="button" variant="destructive" disabled={removingThumb || uploadingThumb} onClick={onRemoveThumb}>
